...
import { useMortgage, AmortizationYear, MortgageSimulation } from '@/hooks/useMortgage'
  ...
  return (
    <div style={{ background: 'var(--s2)', border: '1px solid var(--acc)', borderRadius: 12, padding: '16px 20px', marginTop: 16 }}>
      ...
      <div style={{
        ...
        fontFamily: 'Syne', fontSize: 12, color: 'var(--muted2)', lineHeight: 1.5, marginBottom: 10,
      }}>
      ...
      <div style={{
        ...
        <span style={{ fontWeight: 700, color: 'var(--text)' }}>Spese accessorie stimate: </span>
        circa {formatNumber(result.estimatedAncillaryCosts, 0)} € (notaio, imposte, perizia, istruttoria — stima indicativa, verifica con notaio/banca).
      </div>
  ...
  return (
    <div style={{ background: 'var(--s2)', border: '1px solid var(--border)', borderRadius: 12, padding: '16px 20px' }}>
      ...
      <div style={{ fontFamily: 'Syne', fontSize: 12, color: 'var(--muted)', marginBottom: 14 }}>
        Inserisci i dati dell'immobile e del mutuo richiesto per calcolare la rata, il rapporto rata/reddito e la sostenibilità.
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 10, marginBottom: 12 }}>
        <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          <span style={{ fontFamily: 'Syne', fontSize: 11, color: 'var(--muted2)' }}>Importo immobile (€)</span>
          <input
            value={propertyValue} onChange={e => setPropertyValue(e.target.value)} type="number" placeholder="es. 250000"
        ...
        <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          <span style={{ fontFamily: 'Syne', fontSize: 11, color: 'var(--muted2)' }}>Importo richiesto mutuo (€)</span>
          <input
            value={loanAmount} onChange={e => setLoanAmount(e.target.value)} type="number" placeholder="es. 200000"
            style={{ background: 'var(--s3)', border: '1px solid var(--border)', borderRadius: 8, padding: '10px 12px', color: 'var(--text)', fontFamily: 'JetBrains Mono', fontSize: 13, outline: 'none' }}
          />
        </label>
        <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          ...
          <input
            value={interestRatePct} onChange={e => setInterestRatePct(e.target.value)} type="number" step="0.01" placeholder="es. 3.5"
            style={{ background: 'var(--s3)', border: '1px solid var(--border)', borderRadius: 8, padding: '10px 12px', color: 'var(--text)', fontFamily: 'JetBrains Mono', fontSize: 13, outline: 'none' }}
          />
        </label>
        <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          <span style={{ fontFamily: 'Syne', fontSize: 11, color: 'var(--muted2)' }}>Anni a disposizione</span>
          <input
            value={years} onChange={e => setYears(e.target.value)} type="number" placeholder="es. 25"
            style={{ background: 'var(--s3)', border: '1px solid var(--border)', borderRadius: 8, padding: '10px 12px', color: 'var(--text)', fontFamily: 'JetBrains Mono', fontSize: 13, outline: 'none' }}
          />
        </label>
        <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
