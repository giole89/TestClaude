...
export function AlertsPage() {
  const { alerts, history, addAlert, removeAlert } = useAlertStore()
  ...
  const handleAdd = () => {
    ...
    if (!t || isNaN(v)) return
    addAlert({ ticker: t, type, value: v, fired: false })
    setTicker('')
    setValue('')
  }
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <input
            ...
            style={{
              ...
              fontFamily: 'JetBrains Mono', fontSize: 13, outline: 'none', width: 140,
            }}
          />
          <select
            value={type}
            ...
            style={{
              background: 'var(--s3)', border: '1px solid var(--border)',
              borderRadius: 8, padding: '10px 14px', color: 'var(--text)',
              fontFamily: 'Syne', fontSize: 13, outline: 'none', cursor: 'pointer',
            }}
          >
          ...
          <input
            ...
            style={{
              background: 'var(--s3)', border: '1px solid var(--border)',
              borderRadius: 8, padding: '10px 14px', color: 'var(--text)',
              fontFamily: 'JetBrains Mono', fontSize: 13, outline: 'none', width: 120,
            }}
          />
          <button
            onClick={handleAdd}
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        {alerts.length === 0 && (
          <div style={{ color: 'var(--muted)', fontFamily: 'Syne', fontSize: 13, padding: '20px 0' }}>
            Nessun alert attivo. Aggiungine uno sopra.
          </div>
        )}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {alerts.map(alert => (
            <div key={alert.id} style={{
              ...
              <span style={{ fontFamily: 'JetBrains Mono', fontWeight: 700, fontSize: 14, color: 'var(--acc)', minWidth: 80 }}>
                {alert.ticker}
              </span>
              <span style={{ fontFamily: 'Syne', fontSize: 13, color: 'var(--muted2)', flex: 1 }}>
                {ALERT_LABELS[alert.type]} {alert.value}
                {alert.type === 'change_up' || alert.type === 'change_down' ? '%' : ''}
              </span>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {history.map(alert => (
              <div key={alert.id} style={{
                ...
                <span style={{ fontFamily: 'JetBrains Mono', fontWeight: 700, fontSize: 14, color: 'var(--acc3)', minWidth: 80 }}>
                  {alert.ticker}
                </span>
                <span style={{ fontFamily: 'Syne', fontSize: 13, color: 'var(--muted2)', flex: 1 }}>
                ...
                <span style={{ fontFamily: 'JetBrains Mono', fontSize: 11, color: 'var(--muted)' }}>
                  {alert.firedAt ? formatDateTime(alert.firedAt) : ''}
                </span>
                <span style={{
                  padding: '2px 8px', borderRadius: 6,
                  background: 'var(--acc3)', color: '#07080a',
